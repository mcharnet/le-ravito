...
const ReserverPage: React.FC = () => {
  ...
  return (
    <>
      <Head>
        <title>Réserver - Le Ravito | Table, Événements & Click & Collect à Lyon</title>
        <meta 
          name="description" 
                  ...
                  <div className="bg-white rounded-xl shadow-lg p-8">
                    <div className="mb-6">
                      <h2 className="text-2xl font-bold text-custom-grey mb-2">
                        ...
                        {activeTab === 'click-collect' && 'Commande Click & Collect'}
                      </h2>
                      <p className="text-custom-grey/70">
                        {activeTab === 'table' && 'Réservez votre table pour une expérience optimale'}
                        {activeTab === 'event' && 'Rejoignez nos événements sportifs et ateliers'}
                        {activeTab === 'click-collect' && 'Commandez en ligne et récupérez à l\'heure souhaitée'}
                      </p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        {activeTab === 'event' && (
                          <div>
                            <label htmlFor="eventId" className="block text-sm font-medium text-custom-grey mb-2">
                              Événement *
                            </label>
                            <select
                              id="eventId"
                  ...
                  <div className="bg-white rounded-xl shadow-lg p-6">
                    ...
                    {activeTab === 'event' && (
                      <div className="space-y-3">
                        <p className="text-sm text-custom-grey/70 mb-4">Événements à venir :</p>
                        {upcomingEvents.map(event => (
                          <div key={event.id} className="p-3 bg-gray-50 rounded-lg">
                            <div className="font-medium text-custom-grey text-sm">{event.name}</div>
                            <div className="text-xs text-custom-grey/60 mt-1">
                              {event.date} à {event.time} • {event.price} • {event.spots} places restantes
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    {activeTab === 'click-collect' && (
                      <div className="space-y-3">
                        <p className="text-sm text-custom-grey/70 mb-4">Produits populaires :</p>
                        {popularItems.map((item, index) => (
                          <div key={index} className="flex justify-between text-sm">
                            <span className="text-custom-grey">{item.name}</span>
                            <span className="text-accent-orange font-medium">{item.price}</span>
                          </div>
                        ))}
                  ...
                  <div className="bg-white rounded-xl shadow-lg p-6">
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-custom-grey">Lun-Ven : 6h-20h</span>
                      </div>
                      <a 
                      ...
                      <a 
                        ...
                        className="flex items-center space-x-2 text-accent-orange hover:text-accent-blue transition-colors"
                      >
                        <span>✉️ contact@leravito.fr</span>
                      </a>
                    </div>
                  </div>
                </div>
