...
export const adaptMenuItem = (sanityItem: SanityMenuItem): MenuItem => {
  const menuItem: MenuItem = {
    ...
    image: sanityItem.image
      ? urlFor(sanityItem.image).width(400).height(300).url()
      : "https://images.unsplash.com/photo-1565299624946-b28f40a0ca4b?w=400&h=300&fit=crop&auto=format&q=60",
    category: sanityItem.category?.name || "Autre",
    subcategory: sanityItem.subcategory,
    isAvailable: sanityItem.isAvailable ?? true,
  };

  // Ajouter tag seulement s'il existe pour éviter undefined
  const tag = sanityItem.isVegetarian
    ? "Végétarien"
    : sanityItem.isVegan
      ? "Végan"
      : null;
...
export const adaptEvent = (sanityEvent: SanityEvent): Event => {
  ...
  const event: Event = {
    ...
    image: sanityEvent.image
      ? urlFor(sanityEvent.image).width(400).height(300).url()
      : "https://images.unsplash.com/photo-1565299624946-b28f40a0ca4b?w=400&h=300&fit=crop&auto=format&q=60",
